...
//created for support of fragmented Websocket messages

bool WebSocketsClient::sendBIN(bool first, bool last, uint8_t * payload, size_t length, bool headerToPayload) {
    if(clientIsConnected(&_client)) {
        if(first) {
            return sendFrame(&_client, WSop_binary, payload, length, false, headerToPayload);
        } else if(last) {
            return sendFrame(&_client, WSop_continuation, payload, length, true, headerToPayload);
        } else {
